module RasterizerMaster
  ...
  // does not include lane offsets, but all lanes are relitive to them
  ...
  //
  // RasterizerMaster can control many differnt rasterizers, RasterizerMaster
  ...
  // for furture multi lane functionality. At the moment, I am not implmeneting
  // the multilane functionality. So, this is a unessiary abstraction.
  //
  // rst is active low
  //
  // This system first startes on when the start input wire is hit this triggers
  // the precompution stages of the renderer. The C++ which accomplishes this is
  // pasted below
  //
  ...
  // pixel. These evole as discribed below:
  //
  ...
  //  }
  //
  // This is a convient way to do this, since it is trivial to add extra lanes
  // by adding extra rows, by adding and offset to d0, d1, d2, and i.
  //
